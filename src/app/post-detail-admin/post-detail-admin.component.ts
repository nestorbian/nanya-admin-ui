...
@Component({
  selector: 'app-post-detail-admin',
  templateUrl: './post-detail-admin.component.html',
  styleUrls: ['./post-detail-admin.component.scss']
})
export class PostDetailAdminComponent implements OnInit {
  // 错误提示弹窗
    ...
    this.userService.deletePostById(this.postId).subscribe((data) => {
      if (data['status']) {
        ...
        setTimeout(() => {
          this.successMessage = '';
          this.isShowSuccessTip = false;
          this.router.navigate(['/home-admin/post']);
        }, 1500);
      } else {
        this.errorMessage = data['message'];
        this.showErrorTip();
